...
UserApp.post('/signup', expressAsyncHandler(async (req, res) => {
    try {
        ...
        const {
            userId, name, email, password, age, gender, diabetesType,
            sugarLevels, dietaryPreference, dailyCaloricIntake, foodAllergies,
            mealTypePreference, activityLevel, weight, height
        } = req.body;
        ...
        // ✅ Validate sugarLevels structure
        const formattedSugarLevels = sugarLevels.map(level => ({
            mealType: level.mealType || "initial",
            fastingSugarLevel: level.fastingSugarLevel || -1,
            preMealSugarLevel: level.preMealSugarLevel || -1,
            postMealSugarLevel: level.postMealSugarLevel || -1,
            date: level.date ? new Date(level.date) : new Date() // Ensure valid Date format
        }));

        // ✅ Create new user with corrected structure
        const newUser = new User({
            userId, name, email, password: hashedPassword, age, gender, diabetesType,
            dietaryPreference, dailyCaloricIntake, foodAllergies,
            mealTypePreference, activityLevel, weight, height, BMI,
            sugarLevels: formattedSugarLevels // Store correctly formatted sugar levels
        });
